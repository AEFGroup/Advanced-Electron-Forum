...
function tellafriend() {
    ...
    $message = lang_vars($l['tellafriend_message'], array($topid, (!empty($user['username']) ? $user['username'] : '' )));
    ...
    if (isset($_POST['tellafriend'])) {
        ...
        //before we check for anything, check if the user has already told a friend
        $qresult = makequery("SELECT *
            FROM " . $dbtables['tellafriend'] . "
            WHERE userid='".$user['id']."' AND time > '".time()."'");

        //Is there any such topic
        if (mysql_num_rows($qresult) > 0) {

            //Show a major error and return
            reporterror($l['limit_exceeded'], $l['limit_exceeded_desc']);

            return false;
        }
        else{
            //he is no longer guilty delete it from records
            makequery("DELETE FROM " . $dbtables['tellafriend'] . " WHERE 'userid' = '".$user['id']."' AND 'time' <  '".time()."'");
        }
        ...
        $mail[0]['message'] = lang_vars($l['tellafriend_mail'], array($recipientname, $sendername, unhtmlentities($message)));

        //Pass all Mails to the Mail sending function
        aefmail($mail);
        $interval = time() + $globals['share_limit']*60 ;
        
        //before we redirect, we need to save this bastard's name, so he won't use this as spam for the next 20 minutes
        makequery("INSERT INTO " . $dbtables['tellafriend'] . " (userid, time) VALUES (".$user['id'].",".$interval.")");
